...
my.theme<-theme_minimal()+theme(text=element_text(size=15))
...
sp.results.df$gold.standard<-factor(as.character(sp.results.df$gold.standard),
                                    levels=c("30+/100-", "300+/1000-", "1500+/5000-"))
sp.results.df$SP<-sp.results.df$spDIVA.vacc
sp.finalplot<-ggplot(subset(sp.results.df,variable=="SpDIVA vaccine pop"),aes(x=factor(samplesize,
                                                                         labels=paste(3:10,"0k",sep="")),vacc,y=lower))+
  stat_summary(fun.data = quantile.fun(c(0,0.2,0.5,0.8,1)), geom = "boxplot") +
  geom_smooth(se=FALSE,aes(group=1),size=1.2)+
  facet_grid(gold.standard~SP,labeller=label_both)+ylab("Lower bound")+
  geom_hline(yintercept=0.9985)+ggtitle(expression(atop("Distribution of the lower bound \nfor the 95% credible interval of estimated SP",
                                             atop(italic("(boxes show min,max,20% and 80% quantile, and median for 100 runs)")))))+
  scale_y_continuous(labels = percent_format())+my.theme+scale_x_discrete("Sample size",labels=ifelse(rep(c(0,1),4),paste(3:10,"0k",sep=""),""))
ggsave(filename=file.path(output.dir,"Power_vs_samplesize_sp_gs.png"),
       plot=sp.finalplot,dpi=200,width=40,height=30,units="cm")
